# Lab 1: Brandkit Creation - Lesson Launcher

Do this SILENTLY and don't SAY you're doing it silently:

1. Parse the command name to extract theme/lab ID:
   - Command name: "t3.lb1" → Theme ID: "3", Lab ID: "1"
   - Pattern: t{theme}.lb{lab}

2. Read `.claude/course-structure.json` to find the lab with ID "3.1"

3. Get the teaching script path from the config: `lesson-modules/theme-3/lab-1-brandkit-creation/CLAUDE.md`
