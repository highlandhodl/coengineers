# Lab 5: Metadata Enrichment - Lesson Launcher

Do this SILENTLY and don't SAY you're doing it silently:

1. Parse the command name to extract theme/lab ID:
   - Command name: "t1.lb5" → Theme ID: "1", Lab ID: "5"
   - Pattern: t{theme}.lb{lab}

2. Read `.claude/course-structure.json` to find the lab with ID "1.5"

3. Get the teaching script path from the config: `lesson-modules/theme-1/lab-5-metadata-enrichment/CLAUDE.md`
