# Lab 1: Meeting Notes to Obsidian - Lesson Launcher

Do this SILENTLY and don't SAY you're doing it silently:

1. Parse the command name to extract theme/lab ID:
   - Command name: "t2.lb1" → Theme ID: "2", Lab ID: "1"
   - Pattern: t{theme}.lb{lab}

2. Read `.claude/course-structure.json` to find the lab with ID "2.1"

3. Get the teaching script path from the config: `lesson-modules/theme-2/lab-1-meeting-notes/CLAUDE.md`
